...
def read_row(ws: Worksheet, row: int, header_row: int, mapping: dict[str, int] | None = None) -> dict:
    """把某一行按表头文字读成 {表头: 值} 的字典，给预览界面展示用——公式格子尽量换成算出来的
    结果（见 resolve_cell_value），不是原样把公式文字甩给人看。

    mapping：调用方如果要连着读好几行（比如预览界面给一批改动过的行各生成一份"变化前/变化后"
    快照），应该自己先调一次 column_index_map() 算好、传进来复用，不要让这个函数每读一行都
    自己重新算一遍——column_index_map() 要扫一遍表头那一整行，真实表格如果 max_column 被
    (openpyxl 的已知怪癖：某个很远的格子只是留了点格式) 撑得很大（实测有的表能到 1.6 万+），
    读几十上百行的话，这笔重复扫描的开销会成倍放大，是这个预览功能实测卡顿的一个主要原因。
    不传的话退回自己算一次，兼容只读一行的场景。
    """
    if mapping is None:
