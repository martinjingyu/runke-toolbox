...
class SalesImportPanel(QWidget):
    def __init__(self):
        ...
        note = QLabel(
            ...
            "数量会自动加总。\n\n"
            "【每次导入，4个仓库 × 4个平台，这一天这一列都会整列覆盖，不只是这次提供了数据"
            "的那几个】——这次源数据里有的 SKU 写新数据，没有的 SKU 统统清成 0（哪怕这次根本"
            "没选那个平台的源文件，那个平台当天原有的数据也会被清空）。所以**每次导入都必须"
            "把当天全部源文件（ERP + CG 的 3 份 CSV）一起选上**，少选一份，那份对应平台当天的"
            "真实销量会被一起清空，不会因为没选就保留原样。\n\n"
            "先点「生成预览」看一眼改动（被清空的 SKU 也会在预览里显示"
            "「改之前→0」），确认没问题再点「确认写入」，写入前会自动备份原文件。"
        )
