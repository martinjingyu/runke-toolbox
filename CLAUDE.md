...
- **业务人员口头描述的字段含义，跟这个字段在真实数据里实际的含义，可能对不上——拿真实数据
  ...
  实现（不做任何跨货号库存替代）。
- **openpyxl 插入行/列不会像 Excel 那样自动调整公式里的单元格引用**，而且 `insert_rows()`
  本身很贵——不管插入点在哪，内部都要把整张表所有单元格的坐标先排一次序（见
  `openpyxl/worksheet/worksheet.py` 的 `_move_cells`），在几万行的表上这一步本身就很慢，
  一批发货几十上百笔分摊如果每笔都真插一次行，代价会乘起来（实测能到近一分钟）。
  `shipment_summary.py` 现在的做法是从根上绕开"插入点在表格中间"这件事：待定行拆分之后原地
  不动，只改数量；新插入的"已发货"记录固定放在表格最下面（合计行上面，如果有的话——见
  `_push_tail_row_down`/`_grow_tail_formula`），需要挪动的最多只有合计行这一行，代价是常数，
  不管这一批发货有多少笔分摊、待定行分散在表格哪里都一样快，也就不需要再去追一遍"插入点以下
  所有行的公式引用要 +1"这件事。—— 已经问过用户要不要换成 `xlwings`（遥控真实 Excel 来做，公式
  调整交给 Excel 自己处理）代替这套手写逻辑，用户选择继续用 openpyxl + 手写修正（更快、不依赖
  Excel 安装），如果以后这套手写逻辑又踩到没覆盖的公式写法，可以重新考虑这个选项。
  采购汇总表的日期列原来也是同一套插入逻辑（`purchase_book.py` 的 `find_or_create_date_column`），
  但插日期列比插行/合计行复杂得多——它右边跟着一长串已有列（未出货数量公式、后续日期列……），
  即便小心处理了公式区间/格式/列宽，实际用起来还是出现过插入点右边的公式跟着错位的问题。
  最后改成不再自动插列：日期列必须已经存在于表格里，找不到就直接报错让人工先手动加好这一天
  的列（`purchase_book.py` 的 `require_date_column`），数据永远只是"写进已有的日期列"，不会
  再有"插入之后要重新处理右边所有列"这件事。
