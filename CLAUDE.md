...
- **openpyxl 插入行/列不会像 Excel 那样自动调整公式里的单元格引用**，而且 `insert_rows()`
  ...
  所有行的公式引用要 +1"这件事。插入列同理（采购汇总表插日期列，见 `purchase_book.py` 的
  `find_or_create_date_column`）——插列没有"放在哪里都行"这个自由度，还是要老老实实处理公式
  区间跟着挪的问题，通用规则是：公式里任何行/列号都要按"这一批插入之后它现在挪到第几行/列"
  重新算一遍，不只是"自己引用自己这一行"这一种情况，区间公式如 `SUM(E6:E100)` 的两个边界都要
  分别判断。—— 已经问过用户要不要换成 `xlwings`（遥控真实 Excel 来做，公式调整交给 Excel 自己
  处理）代替这套手写逻辑，用户选择继续用 openpyxl + 手写修正（更快、不依赖 Excel 安装），如果
  以后这套手写逻辑又踩到没覆盖的公式写法，可以重新考虑这个选项。
