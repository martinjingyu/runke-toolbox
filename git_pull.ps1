...
# 双击 / 右键"用 PowerShell 运行"启动的窗口，脚本一结束（不管是正常跑完还是报错）
# 宿主窗口会立刻自己关掉，看不到任何输出、更别说复制报错文字了。这里把整个脚本包一层
# try/catch/finally：出错的话，先把完整的错误信息打印出来，最后不管成功失败都停下来
# 等按一下回车才关窗口，这样至少能看清楚、能复制。
try {
    ...
    if (-not (Test-Path $NAS_SOURCE_DIR)) {
        throw "无法访问共享文件夹 $NAS_SOURCE_DIR。请先在资源管理器地址栏输入 $NAS_UNC 确认可以连接（这台电脑是不是没连上公司网络/没有这个共享文件夹的访问权限），确认能打开之后再重试。"
    }
